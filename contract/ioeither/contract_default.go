...
//
// cert - certificate used to encrypt the contract
// privKey - private key used to sign the environment and workload section
func DefaultEncryptAndSignContract(encrypter Encrypt.Encryption) func(cert []byte) func(privKey []byte) func(Contract.RawMap) IOE.IOEither[error, Contract.RawMap] {
	return func(cert []byte) func([]byte) func(Contract.RawMap) IOE.IOEither[error, Contract.RawMap] {
		return EncryptAndSignContract(
			encrypter.EncryptBasic(cert),
			encrypter.SignDigest,
			encrypter.PubKey,
		)
	}
}
